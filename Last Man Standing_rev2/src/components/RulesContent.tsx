    ...
    content: (
      <>
        <ul className="list-inside list-disc space-y-1 text-gray-700">
          ...
          <li>
            Pay via <a href="https://revolut.me/jasons78" className="text-rvr-maroon font-medium underline hover:no-underline" target="_blank" rel="noopener noreferrer">revolut.me/jasons78</a> (0833746682) where possible, or cash to Rivervalley coach representatives (Dave, Aidan, Rui or Stephen).
          </li>
      ...
      {sections.map(({ id, title, content }) => (
        ...
        >
          <button
            ...
            className="flex w-full items-center justify-between px-4 py-3 text-left font-medium text-gray-900 hover:bg-rvr-maroon-muted/50"
          >
            {title}
            <span className="text-rvr-maroon">
              {openId === id ? "−" : "+"}
            </span>
          </button>
          {openId === id && (
            <div className="border-t border-gray-200 bg-rvr-maroon-muted/30 px-4 py-3">
              {content}
            </div>
          )}
        </div>
