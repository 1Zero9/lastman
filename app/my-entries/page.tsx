...
export default async function MyEntriesPage() {
  ...
  if (!participants.length) {
    const [membership, account] = await Promise.all([
      prisma.competitionMember.findFirst({ where: { userId: user.id, role: { in: ["OWNER", "ADMIN"] }, competition: { status: { not: "ARCHIVED" } } }, select: { id: true } }),
      prisma.user.findUnique({ where: { id: user.id }, select: { organiserApprovedAt: true } }),
    ]);
    return <div className="rounded-2xl bg-surface p-6 ring-1 ring-border"><h1 className="text-2xl font-bold text-text">My entries</h1><p className="mt-3 text-text-secondary">No competition entries are linked to this account yet. Playing? Ask your organiser to add you using this email address.</p>{membership ? <Link href="/admin" className="mt-5 inline-flex rounded-xl bg-primary px-5 py-3 font-semibold text-white">Open the admin area</Link> : account?.organiserApprovedAt ? <Link href="/admin/setup" className="mt-5 inline-flex rounded-xl bg-primary px-5 py-3 font-semibold text-white">Set up your fundraiser</Link> : null}</div>;
  }
  ...
  return <div className="space-y-6">{welcomeParticipant && <ClubWelcome competitionId={welcomeParticipant.competitionId} playerName={welcomeParticipant.name} clubName={welcomeParticipant.competition.clubName!} clubWebsite={welcomeParticipant.competition.clubWebsite} clubColor={welcomeParticipant.competition.clubColor} welcomeMessage={welcomeParticipant.competition.welcomeMessage} />}<div><p className="text-sm font-semibold uppercase tracking-wide text-primary">Member area</p><h1 className="mt-2 text-3xl font-bold text-text">My entries</h1></div>{cards}</div>;
}
