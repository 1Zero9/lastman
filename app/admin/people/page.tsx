...
export default async function PeoplePage() {
  ...
  const buyBackEnabled = Boolean((season.rules as { buyBack?: { enabled?: boolean } }).buyBack?.enabled) && season.status !== "COMPLETED";
  const earlyGameweekIds = new Set((await prisma.gameweek.findMany({ where: { seasonId: season.id, number: { lte: 2 } }, select: { id: true } })).map((gw) => gw.id));
